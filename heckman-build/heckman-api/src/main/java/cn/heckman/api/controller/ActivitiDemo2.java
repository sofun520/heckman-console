...
import cn.heckman.framework.utils.ActivitiConstant;
...
public class ActivitiDemo2 {
	...
	public ResponseData deploy() {
		...
		try {
			repositoryService// 与流程定义和部署对象相关的service
					...
					.name("员工请假流程")// 添加部署的名称
					.addClasspathResource("bpmn/leaveProcess.bpmn")// classpath的资源中加载，一次只能加载
					.addClasspathResource("bpmn/leaveProcess.png")// classpath的资源中加载，一次只能加载
					.deploy();// 完成部署
	...
	public ResponseData startProcessInstance() {
		...
		try {
			...
			t.setTaskName(ActivitiConstant
					.getTaskName(ActivitiConstant.LEAVE_PROCESS));
			t.setTaskProId(processInstance.getId());
	...
	public ResponseData completeMyPersonTask(HttpServletRequest request) {
		...
		try {
			...
			taskService.addComment(taskId, processInstancesId, taskContent);
