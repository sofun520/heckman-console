...
/* Controllers */
app.factory('workFlowService', function smsService($http) {
	return {
		loadList : function loadList(query, callback) {
			$http({
				method : "post",
				url : config.urlConfig.rootUrl + "api/workFlow/query",
				data : query,
				headers : {
...
app.controller('workFlowCtrl', [
		'$scope',
		'workFlowService',
		'$compile',
		'commonUtil',
		function($scope, workFlowService, $compile, commonUtil) {
			...
			$scope.loadList = function() {
				var obj = {};
				$scope.param.startUserId=1;
				obj.pageBean = $scope.pageBean;
				obj.param = $scope.param;
				workFlowService.loadList(JSON.stringify(obj), function(error, data) {
					if (!error && data.code == 0) {
						if (data.data.length > 0) {
							...
							for (var i = 0; i < data.pageCount; i++) {
								html += '<li><a href="" ng-click="changePage('
										+ (i + 1) + ')">' + (i + 1)
										+ '</a></li>';
							}
					...
					buttons : {
						tryAgain : {
							...
							action : function() {
								...
								/*
								 * smsService.saveSmsTemp(JSON.stringify(param),
								 * function (error, data) { if (!error &&
								 * data.code == 0) { $.alert({title: '系统提示',
								 * content: '添加成功'}); } });
								 */
							}
						},
					...
					buttons : {
						tryAgain : {
							...
							action : function() {
								...
								smsService.delSmsTemp(JSON.stringify(param),
										function(error, data) {
											if (!error && data.code == 0) {
												$.alert({
													title : '系统提示',
